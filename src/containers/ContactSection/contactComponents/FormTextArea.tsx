...
interface FormTextAreaProps {
    ...
    isError: boolean,
    handleChange: (event: React.ChangeEvent<HTMLTextAreaElement>, tag: string) => void,
}
...
const FormTextArea = ({ fieldValue, tag, inputRef, isError, handleChange }: FormTextAreaProps) => {
    return (
        <StyledTextArea fieldValue={fieldValue} isError={isError}>
            ...
            {isError ? <p>Message cannot be empty</p> : <p>{inputRef.current?.textLength ?? 0}/500</p>}
        </StyledTextArea>
    );
};
